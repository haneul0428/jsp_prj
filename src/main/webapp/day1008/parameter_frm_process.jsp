...
<%-- console.log("<%= request.getMethod() %>방식의 요청"); --%>
...
<h2><%= method %>방식의 요청</h2>
<%
	//POST방식일때만 동작: 입력스트림의 모든 charset을 변환
	request.setCharacterEncoding("UTF-8");
	
	//이름이 유일한 web parameter 받기(request 내장객체를 사용)
	///parameter_frm_process.jsp?text=f&pass=afs&chk=on&hid=%EC%88%A8%EA%B9%80%EA%B0%92&sel=JDBC&ta=qwerasdf
	...
	if("POST".equals(method)){
		//String class를 사용한 변환(문자열을 byte[]로 쪼개서 인코딩을 수행)
		text = new String(text.getBytes("8859_1"), "UTF-8");
		
		//URLEncoder, URLDecoder 사용 가능
